...
import loadMoreDataMethod from "@salesforce/apex/RelatedListController.loadMoreData";
...
export default class RelatedListHelper {
    ...
    fetchMoreData(state) {
        let jsonData = Object.assign({}, state)
        jsonData = JSON.stringify(jsonData)
        return loadMoreDataMethod({ jsonData })
            .then(response => {
                const data = JSON.parse(response);
    ...
    processData(data, state){
        ...
        if (records.length > state.numberOfRecords) {
            records.pop()
            data.title = ` (${state.numberOfRecords}+)`
        } else {
            data.title = ` (${Math.min(state.numberOfRecords, records.length)})`
        }     
        return data
    }
